...
define('encodeji', function(){
    ...
    // }, ... ]
    var data = unpackOnFirstUse(EMOJI_DATA_JSON_STRING); // eslint-disable-line

    function unpackOnFirstUse(s){
        var arr;
        return function(){
            if(arr) return arr;
            arr = JSON.parse(s);
    ...
    Encodeji.prototype.buildShortNameIndex = function(){
        ...
        this.data().forEach(function(d){
            // ignore duplicate names. There are some cases where these are
            // valid in the data, such as :man-woman-boy: which can be composed
            // as either the legacy 'family' codepoint 1F46A, or as the
            // combination 1F468-200D-1F469-200D-1F466.
            if(checkForDuplicateShortName(d.short_name, d)){
                // if the existing duplicate is not a primary name, then
                // continue so this one replaces it:
                if(dataByShortName[d.short_name].short_name === d.short_name){
                    return;
                }
            }
            ...
            if(d.short_names && d.short_names.length){
                d.short_names.forEach(function(n){
                    ...
                    // .short_names:
                    if(n !== d.short_name){
                        if(!checkForDuplicateShortName(n, d)){
                            dataByShortName[n] = d;
